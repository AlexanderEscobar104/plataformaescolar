...
function TipoInasistenciasPage() {
  ...
  const loadTipos = useCallback(async () => {
    setLoading(true)
    try {
      const snap = await getDocs(collection(db, 'tipo_inasistencias'))
  ...
  const handleSubmit = async (event) => {
    ...
    if (!trimmedNombre) {
      setFeedback('Debes ingresar el nombre del tipo de inasistencia.')
      return
    }
    ...
    try {
      setSaving(true)
      if (editingTipo) {
        await updateDoc(doc(db, 'tipo_inasistencias', editingTipo.id), {
        ...
        setFeedback('Tipo de inasistencia actualizado correctamente.')
      } else {
        await addDoc(collection(db, 'tipo_inasistencias'), {
          nombre: trimmedNombre,
          descripcion: form.descripcion.trim(),
          estado: form.estado,
          esIntegrado: false,
        ...
        setFeedback('Tipo de inasistencia creado correctamente.')
      }
      resetForm()
      await loadTipos()
    } catch {
      setFeedback('No fue posible guardar el tipo de inasistencia.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true)
      await deleteDoc(doc(db, 'tipo_inasistencias', tipoToDelete.id))
      setFeedback('Tipo de inasistencia eliminado correctamente.')
      setTipoToDelete(null)
      await loadTipos()
    } catch {
      setFeedback('No fue posible eliminar el tipo de inasistencia.')
    } finally {
      setDeleting(false)
    }
  }
  ...
  return (
    <section className="evaluations-page">
      <div className="students-header">
        <div>
          <h2>Tipos de inasistencia</h2>
          <p>Gestiona los tipos de inasistencia para los reportes de estudiantes.</p>
        </div>
        {canCreate && (
        <button
          type="submit"
          form="tipo-inasistencias-form"
          className="button"
          disabled={saving}
        >
      ...
      <div className="home-left-card evaluations-card">
        <h3>{editingTipo ? 'Editar tipo de inasistencia' : 'Nuevo tipo de inasistencia'}</h3>
        <form id="tipo-inasistencias-form" className="form evaluation-create-form" onSubmit={handleSubmit}>
          <fieldset className="form-fieldset" disabled={saving}>
            ...
            <label htmlFor="ti-descripcion" className="evaluation-field-full">
              ...
              <input
                ...
                placeholder="Describe brevemente este tipo de inasistencia"
              />
            </label>
      ...
      <div className="home-left-card evaluations-card" style={{ width: '100%' }}>
        <section style={{ marginTop: '0' }}>
          <h3>Lista de tipos de inasistencia</h3>
          ...
          {loading ? (
            <p>Cargando tipos de inasistencia...</p>
          ) : (
              ...
              <table className="students-table">
                ...
                <tbody>
                  {filteredRows.length === 0 && (
                    <tr>
                      <td colSpan={(canEdit || canDelete) ? 4 : 3}>No hay tipos de inasistencia para mostrar.</td>
                    </tr>
                  )}
                    ...
                    <tr key={item.id}>
                      ...
                      <td data-label="Acciones" className="student-actions">
                        ...
                        <button
                          ...
                          title="Editar"
                          aria-label="Editar tipo de inasistencia"
                        >
                        ...
                        <button
                          ...
                          title="Eliminar"
                          aria-label="Eliminar tipo de inasistencia"
                        >
                          <svg viewBox="0 0 24 24" aria-hidden="true">
...
export default TipoInasistenciasPage
