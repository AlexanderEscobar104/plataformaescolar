...
function PermisosPage() {
  ...
  const handleTypeChange = (e) => {
    ...
    const tipo = tiposPermiso.find((t) => t.id === typeId)
    setForm((prev) => ({
      ...prev,
      tipoId: typeId,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await updateDoc(doc(db, 'permisos', editingId), payload)
        setFeedback('Permiso actualizado correctamente.')
      } else {
        payload.creadoEn = serverTimestamp()
        payload.creadoPorUid = user?.uid || ''
        await addDoc(collection(db, 'permisos'), payload)
        setFeedback('Permiso reportado correctamente.')
      }
      setForm({
        estudianteId: '',
        estudianteNombre: '',
      ...
      setValidationError('No fue posible reportar el permiso.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'permisos', itemToDelete.id))
      setFeedback('Permiso eliminado correctamente.')
      setItemToDelete(null)
      await loadData()
    } catch (error) {
      console.error('Error deleting permiso:', error)
      setValidationError('No fue posible eliminar el permiso.')
    } finally {
      setDeleting(false)
    }
  }
  ...
  const handleSaveNewSoporte = async (e) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'permisos', selectedItemForSoporte.id), {
        soporteUrl: url,
  ...
  return (
    <section className="evaluations-page">
      <div className="students-header">
        <div>
          <h2>Reportar Permisos</h2>
          <p>Registra y consulta los permisos del personal y sus soportes.</p>
        </div>
      </div>
        ...
        <form className="form evaluation-create-form" onSubmit={handleSubmit}>
          <fieldset className="form-fieldset" disabled={saving}>
            <label htmlFor="ina-estudiante" className="evaluation-field-full">
              ...
              <SearchableSelect
                id="ina-estudiante"
                options={estudiantes.map(est => ({ value: est.id, label: `${est.name} (${est.role})` }))}
            ...
            <div className="modal-actions evaluation-field-full">
              ...
              <button type="submit" className="button" disabled={saving}>
                {saving ? 'Guardando...' : editingId ? 'Guardar Cambios' : 'Reportar Permiso'}
              </button>
            </div>
          </fieldset>
        </form>
      ...
      {itemToDelete && (
        <div className="modal-overlay" role="presentation">
          <div className="modal-card" role="dialog" aria-modal="true" aria-label="Confirmar eliminación">
            ...
            <p>
              ¿Estás seguro de que deseas eliminar este permiso para{' '}
      ...
      {showAddSoporteModal && selectedItemForSoporte && (
        <div className="modal-overlay" role="presentation">
          <div className="modal-card" role="dialog" aria-modal="true" aria-label="Agregar soporte">
            ...
            <p>Añade el soporte faltante para este permiso.</p>
