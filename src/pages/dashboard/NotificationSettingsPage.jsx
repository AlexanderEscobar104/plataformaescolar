...
function NotificationSettingsPage() {
  ...
  const canManage =
    hasPermission(PERMISSION_KEYS.CONFIG_NOTIFICATIONS_MANAGE) || hasPermission(PERMISSION_KEYS.PERMISSIONS_MANAGE)
    ...
    const loadData = async () => {
      ...
      try {
        const [rolesSnapshot, settingsSnapshot] = await Promise.all([
          ...
          userNitRut ? getDoc(doc(db, 'configuracion', `notifications_roles_${userNitRut}`)) : Promise.resolve(null),
        ])
        ...
        setFeedback('No fue posible cargar la configuracion de notificaciones.')
      } finally {
        setLoading(false)
      }
    }
  ...
  const saveSettings = async () => {
    ...
    try {
      setSaving(true)
      await setDocTracked(
        doc(db, 'configuracion', `notifications_roles_${userNitRut}`),
        {
          roleMatrix,
          studentGroupMatrix,
      ...
      setFeedback('Configuracion de notificaciones guardada correctamente.')
    } catch {
      setFeedback('No fue posible guardar la configuracion de notificaciones.')
    } finally {
      setSaving(false)
    }
  }
    ...
    <section className="dashboard-module-shell settings-module-shell">
      <div className="dashboard-module-hero">
        <div className="dashboard-module-hero-copy">
          ...
          <p>Define que roles pueden enviar notificaciones a otros roles.</p>
        </div>
      ...
      <div className="students-header member-module-header">
        <div className="member-module-header-copy">
          ...
          <p>Controla el alcance de las notificaciones entre roles y grupos estudiantiles.</p>
        </div>
