...
function ChatSettingsPage() {
    ...
    const loadData = async () => {
      ...
      try {
        const [rolesSnapshot, settingsSnapshot] = await Promise.all([
          ...
          userNitRut ? getDoc(doc(db, 'configuracion', `chat_roles_${userNitRut}`)) : Promise.resolve(null),
        ])
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      setFeedback('Configuracion de chat guardada correctamente.')
    } catch {
      setFeedback('No fue posible guardar la configuracion de chat.')
    } finally {
      setSaving(false)
    }
  }
