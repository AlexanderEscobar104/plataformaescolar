const PERMISSION_KEYS = {
  USERS_VIEW: 'users_view',
  USERS_DELETE: 'users_delete',
  USERS_ASSIGN_ROLE: 'users_assign_role',
  USERS_CHANGE_STATE: 'users_change_state',
  MEMBERS_MANAGE: 'members_manage',
  MEMBERS_STUDENTS_VIEW: 'members_students_view',
  MEMBERS_STUDENTS_CREATE: 'members_students_create',
  MEMBERS_STUDENTS_EDIT: 'members_students_edit',
  MEMBERS_STUDENTS_DELETE: 'members_students_delete',
  MEMBERS_STUDENTS_MANAGE: 'members_students_manage',
  MEMBERS_PROFESORES_VIEW: 'members_profesores_view',
  MEMBERS_PROFESORES_CREATE: 'members_profesores_create',
  MEMBERS_PROFESORES_EDIT: 'members_profesores_edit',
  MEMBERS_PROFESORES_DELETE: 'members_profesores_delete',
  MEMBERS_PROFESORES_MANAGE: 'members_profesores_manage',
  MEMBERS_DIRECTIVOS_VIEW: 'members_directivos_view',
  MEMBERS_DIRECTIVOS_CREATE: 'members_directivos_create',
  MEMBERS_DIRECTIVOS_EDIT: 'members_directivos_edit',
  MEMBERS_DIRECTIVOS_DELETE: 'members_directivos_delete',
  MEMBERS_DIRECTIVOS_MANAGE: 'members_directivos_manage',
  MEMBERS_ASPIRANTES_VIEW: 'members_aspirantes_view',
  MEMBERS_ASPIRANTES_CREATE: 'members_aspirantes_create',
  MEMBERS_ASPIRANTES_EDIT: 'members_aspirantes_edit',
  MEMBERS_ASPIRANTES_DELETE: 'members_aspirantes_delete',
  MEMBERS_ASPIRANTES_MANAGE: 'members_aspirantes_manage',
  MEMBERS_ACUDIENTES_VIEW: 'members_acudientes_view',
  MEMBERS_ACUDIENTES_CREATE: 'members_acudientes_create',
  MEMBERS_ACUDIENTES_EDIT: 'members_acudientes_edit',
  MEMBERS_ACUDIENTES_DELETE: 'members_acudientes_delete',
  MEMBERS_ACUDIENTES_MANAGE: 'members_acudientes_manage',
  ACUDIENTE_PORTAL_VIEW: 'acudiente_portal_view',
  ACUDIENTE_BOLETINES_VIEW: 'acudiente_boletines_view',
  ACUDIENTE_ASISTENCIA_VIEW: 'acudiente_asistencia_view',
  ACUDIENTE_PERMISOS_CREATE: 'acudiente_permisos_create',
  ACUDIENTE_TAREAS_VIEW: 'acudiente_tareas_view',
  ACUDIENTE_HORARIO_VIEW: 'acudiente_horario_view',
  ACUDIENTE_PAGOS_VIEW: 'acudiente_pagos_view',
  ACUDIENTE_DOCUMENTOS_VIEW: 'acudiente_documentos_view',
  ACUDIENTE_MESSAGES_SEND: 'acudiente_messages_send',
  ACUDIENTE_NOTIFICATIONS_VIEW: 'acudiente_notifications_view',
  MEMBERS_DYNAMIC_MENUS_VIEW: 'members_dynamic_menus_view',
  EMPLEADOS_VIEW: 'empleados_view',
  EMPLEADOS_CREATE: 'empleados_create',
  EMPLEADOS_EDIT: 'empleados_edit',
  EMPLEADOS_DELETE: 'empleados_delete',
  EMPLEADOS_MANAGE: 'empleados_manage',
  PLANTEL_VIEW: 'plantel_view',
  PLANTEL_MANAGE: 'plantel_manage',
  ACADEMIC_SETUP_MANAGE: 'academic_setup_manage',
  EVENTS_MANAGE: 'events_manage',
  CIRCULARS_MANAGE: 'circulars_manage',
  VOTACIONES_MANAGE: 'votaciones_manage',
  ENCUESTAS_MANAGE: 'encuestas_manage',
  ACUDIENTE_VOTACIONES_VIEW: 'acudiente_votaciones_view',
  ACUDIENTE_ENCUESTAS_VIEW: 'acudiente_encuestas_view',
  SUBJECTS_MANAGE: 'subjects_manage',
  REPORTS_VIEW: 'reports_view',
  MANAGEMENT_DASHBOARD_VIEW: 'management_dashboard_view',
  PAYMENTS_VIEW: 'payments_view',
  PAYMENTS_IMPUESTOS_MANAGE: 'payments_impuestos_manage',
  PAYMENTS_RESOLUCIONES_MANAGE: 'payments_resoluciones_manage',
  PAYMENTS_CAJA_MANAGE: 'payments_caja_manage',
  PAYMENTS_DATOS_COBRO_MANAGE: 'payments_datos_cobro_manage',
  PAYMENTS_ITEM_COBRO_MANAGE: 'payments_item_cobro_manage',
  PAYMENTS_SERVICIOS_COMPLEMENTARIOS_MANAGE: 'payments_servicios_complementarios_manage',
  ADMISSIONS_CRM_VIEW: 'admissions_crm_view',
  ADMISSIONS_CRM_MANAGE: 'admissions_crm_manage',
  ADMISSIONS_FOLLOWUPS_MANAGE: 'admissions_followups_manage',
  ADMISSIONS_INTERVIEWS_MANAGE: 'admissions_interviews_manage',
  ADMISSIONS_TASKS_MANAGE: 'admissions_tasks_manage',
  ADMISSIONS_REPORTS_VIEW: 'admissions_reports_view',
  ADMISSIONS_CONVERT_ENROLLMENT: 'admissions_convert_enrollment',
  WHATSAPP_MODULE_VIEW: 'whatsapp_module_view',
  WHATSAPP_SEND: 'whatsapp_send',
  WHATSAPP_TEMPLATES_MANAGE: 'whatsapp_templates_manage',
  WHATSAPP_CAMPAIGNS_MANAGE: 'whatsapp_campaigns_manage',
  WHATSAPP_SETTINGS_MANAGE: 'whatsapp_settings_manage',
  SMS_MODULE_VIEW: 'sms_module_view',
  SMS_SEND: 'sms_send',
  SMS_HISTORY_VIEW: 'sms_history_view',
  SMS_TEMPLATES_MANAGE: 'sms_templates_manage',
  SMS_SETTINGS_MANAGE: 'sms_settings_manage',
  DESEMPENO_MODULE_VIEW: 'desempeno_module_view',
  DESEMPENO_DASHBOARD_VIEW: 'desempeno_dashboard_view',
  DESEMPENO_PERIODS_VIEW: 'desempeno_periods_view',
  DESEMPENO_PERIODS_CREATE: 'desempeno_periods_create',
  DESEMPENO_PERIODS_EDIT: 'desempeno_periods_edit',
  DESEMPENO_PERIODS_CLOSE: 'desempeno_periods_close',
  DESEMPENO_TEMPLATES_VIEW: 'desempeno_templates_view',
  DESEMPENO_TEMPLATES_CREATE: 'desempeno_templates_create',
  DESEMPENO_TEMPLATES_EDIT: 'desempeno_templates_edit',
  DESEMPENO_TEMPLATES_DELETE: 'desempeno_templates_delete',
  DESEMPENO_ASSIGNMENTS_VIEW: 'desempeno_assignments_view',
  DESEMPENO_ASSIGNMENTS_CREATE: 'desempeno_assignments_create',
  DESEMPENO_ASSIGNMENTS_EDIT: 'desempeno_assignments_edit',
  DESEMPENO_ASSIGNMENTS_DELETE: 'desempeno_assignments_delete',
  DESEMPENO_EVALUATIONS_VIEW: 'desempeno_evaluations_view',
  DESEMPENO_EVALUATIONS_CREATE: 'desempeno_evaluations_create',
  DESEMPENO_EVALUATIONS_EDIT: 'desempeno_evaluations_edit',
  DESEMPENO_EVALUATIONS_SUBMIT: 'desempeno_evaluations_submit',
  DESEMPENO_RESULTS_VIEW: 'desempeno_results_view',
  DESEMPENO_RESULTS_CREATE: 'desempeno_results_create',
  DESEMPENO_RESULTS_EDIT: 'desempeno_results_edit',
  DESEMPENO_RESULTS_APPROVE: 'desempeno_results_approve',
  DESEMPENO_IMPROVEMENT_VIEW: 'desempeno_improvement_view',
  DESEMPENO_IMPROVEMENT_CREATE: 'desempeno_improvement_create',
  DESEMPENO_IMPROVEMENT_EDIT: 'desempeno_improvement_edit',
  DESEMPENO_IMPROVEMENT_CLOSE: 'desempeno_improvement_close',
  DESEMPENO_HISTORY_VIEW: 'desempeno_history_view',
  DESEMPENO_REPORTS_VIEW: 'desempeno_reports_view',
  DESEMPENO_REPORTS_EXPORT: 'desempeno_reports_export',
  DESEMPENO_OWN_VIEW: 'desempeno_own_view',
  DESEMPENO_SELF_EVALUATE: 'desempeno_self_evaluate',
  CERTIFICADOS_VIEW: 'certificados_view',
  CERTIFICADOS_GENERATE: 'certificados_generate',
  CERTIFICADOS_ACADEMIC_YEAR_EDIT: 'certificados_academic_year_edit',
  BOLETINES_VIEW: 'boletines_view',
  BOLETINES_EDIT: 'boletines_edit',
  BOLETINES_GENERATE: 'boletines_generate',
  SCHEDULE_VIEW: 'schedule_view',
  CONFIG_CHAT_MANAGE: 'config_chat_manage',
  CONFIG_MAIL_SERVER_MANAGE: 'config_mail_server_manage',
  CONFIG_MESSAGES_MANAGE: 'config_messages_manage',
  CONFIG_NOTIFICATIONS_MANAGE: 'config_notifications_manage',
  CONFIG_REPORT_TYPES_MANAGE: 'config_report_types_manage',
  CONFIG_CERTIFICADOS_TEMPLATES_MANAGE: 'config_certificados_templates_manage',
  CONFIG_BOLETINES_STRUCTURE_MANAGE: 'config_boletines_structure_manage',
  CONFIG_TIPO_PERMISOS_MANAGE: 'config_tipo_permisos_manage',
  CONFIG_TIPO_INASISTENCIAS_MANAGE: 'config_tipo_inasistencias_manage',
  CONFIG_TIPO_CERTIFICADO_MANAGE: 'config_tipo_certificado_manage',
  CONFIG_TIPO_EMPLEADO_MANAGE: 'config_tipo_empleado_manage',
  CONFIG_LINKED_DEVICES_VIEW: 'config_linked_devices_view',
  ANNOUNCEMENTS_MANAGE: 'announcements_manage',
  NOTIFICATIONS_CREATE: 'notifications_create',
  MESSAGES_DELETE: 'messages_delete',
  MESSAGES_SEND: 'messages_send',
  MESSAGES_REPLY: 'messages_reply',
  MESSAGES_READ_RECEIPTS_VIEW: 'messages_read_receipts_view',
  CHAT_ONLINE_VIEW: 'chat_online_view',
  TASKS_VIEW: 'tasks_view',
  TASKS_CREATE: 'tasks_create',
  TASKS_EDIT: 'tasks_edit',
  TASKS_DELETE: 'tasks_delete',
  TASKS_REPLY: 'tasks_reply',
  EVALUATIONS_VIEW: 'evaluations_view',
  EVALUATIONS_MANAGE: 'evaluations_manage',
  EVALUATIONS_CREATE: 'evaluations_create',
  EVALUATIONS_EDIT: 'evaluations_edit',
  EVALUATIONS_DELETE: 'evaluations_delete',
  EVALUATIONS_FOLLOW_UP: 'evaluations_follow_up',
  EVALUATIONS_TAKE: 'evaluations_take',
  EVALUATIONS_GRADE: 'evaluations_grade',
  SCHEDULE_EDIT: 'schedule_edit',
  PERMISSIONS_MANAGE: 'permissions_manage',
  ROLES_MANAGE: 'roles_manage',
  BULK_UPLOAD_MANAGE: 'bulk_upload_manage',
  EXPORT_EXCEL: 'export_excel',
  INASISTENCIAS_VIEW: 'inasistencias_view',
  INASISTENCIAS_CREATE: 'inasistencias_create',
  INASISTENCIAS_EDIT: 'inasistencias_edit',
  INASISTENCIAS_DELETE: 'inasistencias_delete',
  ASISTENCIA_VIEW: 'asistencia_view',
  ASISTENCIA_CONFIG_MANAGE: 'asistencia_config_manage',
  ASISTENCIA_DELETE: 'asistencia_delete',
  PERMISOS_VIEW: 'permisos_view',
  PERMISOS_CREATE: 'permisos_create',
  PERMISOS_EDIT: 'permisos_edit',
  PERMISOS_DELETE: 'permisos_delete',
  STORAGE_MANAGE: 'storage_manage',
}

const PROTECTED_ROLE_VALUES = ['administrador', 'directivo', 'profesor', 'estudiante', 'aspirante', 'acudiente']

const ROLE_OPTIONS = [
  { value: 'administrador', label: 'Administrador' },
  { value: 'directivo', label: 'Directivo' },
  { value: 'profesor', label: 'Profesor' },
  { value: 'estudiante', label: 'Estudiante' },
  { value: 'aspirante', label: 'Aspirante' },
  { value: 'acudiente', label: 'Acudiente' },
]

/**
 * Combines base roles with custom roles loaded from Firestore.
 * @param {Array<{id: string, name: string, status: string}>} customRoles
 * @returns {Array<{value: string, label: string}>}
 */
function buildAllRoleOptions(customRoles = []) {
  const custom = customRoles
    .filter((r) => r.status !== 'inactivo')
    .map((r) => {
      const label = String(r.name || '').trim()
      return {
        value: label.toLowerCase(),
        label,
      }
    })
    .filter((role) => role.value && role.label)

  const seen = new Set()
  return [...ROLE_OPTIONS, ...custom].filter((role) => {
    const normalizedValue = String(role.value || '').trim().toLowerCase()
    if (!normalizedValue || seen.has(normalizedValue)) {
      return false
    }
    seen.add(normalizedValue)
    return true
  })
}

const PERMISSIONS_CATALOG = [
  {
    group: 'Usuarios',
    key: PERMISSION_KEYS.USERS_VIEW,
    label: 'Ver usuarios',
    description: 'Permite visualizar el modulo de usuarios.',
  },
  {
    group: 'Usuarios',
    key: PERMISSION_KEYS.USERS_DELETE,
    label: 'Eliminar usuarios',
    description: 'Permite eliminar usuarios desde el listado.',
  },
  {
    group: 'Usuarios',
    key: PERMISSION_KEYS.USERS_ASSIGN_ROLE,
    label: 'Asignar rol',
    description: 'Permite cambiar el rol de un usuario.',
  },
  {
    group: 'Usuarios',
    key: PERMISSION_KEYS.USERS_CHANGE_STATE,
    label: 'Cambiar estado',
    description: 'Permite cambiar el estado (activo/inactivo) de un usuario.',
  },
  {
    group: 'Reportes',
    key: PERMISSION_KEYS.MANAGEMENT_DASHBOARD_VIEW,
    label: 'Ver dashboard gerencial',
    description: 'Permite acceder al tablero ejecutivo con indicadores gerenciales.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_STUDENTS_VIEW,
    label: 'Ver estudiantes',
    description: 'Permite visualizar el modulo de estudiantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_STUDENTS_CREATE,
    label: 'Crear estudiantes',
    description: 'Permite crear estudiantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_STUDENTS_EDIT,
    label: 'Editar estudiantes',
    description: 'Permite editar estudiantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_STUDENTS_DELETE,
    label: 'Eliminar estudiantes',
    description: 'Permite eliminar estudiantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_PROFESORES_VIEW,
    label: 'Ver profesores',
    description: 'Permite visualizar el modulo de profesores.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_PROFESORES_CREATE,
    label: 'Crear profesores',
    description: 'Permite crear profesores.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_PROFESORES_EDIT,
    label: 'Editar profesores',
    description: 'Permite editar profesores.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_PROFESORES_DELETE,
    label: 'Eliminar profesores',
    description: 'Permite eliminar profesores.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_DIRECTIVOS_VIEW,
    label: 'Ver directivos',
    description: 'Permite visualizar el modulo de directivos.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_DIRECTIVOS_CREATE,
    label: 'Crear directivos',
    description: 'Permite crear directivos.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_DIRECTIVOS_EDIT,
    label: 'Editar directivos',
    description: 'Permite editar directivos.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_DIRECTIVOS_DELETE,
    label: 'Eliminar directivos',
    description: 'Permite eliminar directivos.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ASPIRANTES_VIEW,
    label: 'Ver aspirantes',
    description: 'Permite visualizar el modulo de aspirantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ASPIRANTES_CREATE,
    label: 'Crear aspirantes',
    description: 'Permite crear aspirantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ASPIRANTES_EDIT,
    label: 'Editar aspirantes',
    description: 'Permite editar aspirantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ASPIRANTES_DELETE,
    label: 'Eliminar aspirantes',
    description: 'Permite eliminar aspirantes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ACUDIENTES_VIEW,
    label: 'Ver acudientes',
    description: 'Permite visualizar el modulo de acudientes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ACUDIENTES_CREATE,
    label: 'Crear acudientes',
    description: 'Permite crear acudientes con acceso al portal.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ACUDIENTES_EDIT,
    label: 'Editar acudientes',
    description: 'Permite editar la informacion de acudientes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.MEMBERS_ACUDIENTES_DELETE,
    label: 'Eliminar acudientes',
    description: 'Permite eliminar o desactivar acudientes.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_PORTAL_VIEW,
    label: 'Portal de acudiente',
    description: 'Permite al acudiente ingresar a su portal y consultar informacion general.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_BOLETINES_VIEW,
    label: 'Acudiente ver boletines',
    description: 'Permite al acudiente consultar boletines de estudiantes vinculados.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_ASISTENCIA_VIEW,
    label: 'Acudiente ver asistencia',
    description: 'Permite al acudiente consultar asistencia e inasistencias.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_PERMISOS_CREATE,
    label: 'Acudiente solicitar permisos',
    description: 'Permite al acudiente solicitar permisos para estudiantes vinculados.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_TAREAS_VIEW,
    label: 'Acudiente ver tareas',
    description: 'Permite al acudiente consultar tareas del estudiante.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_HORARIO_VIEW,
    label: 'Acudiente ver horario',
    description: 'Permite al acudiente consultar el horario del estudiante.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_PAGOS_VIEW,
    label: 'Acudiente ver pagos',
    description: 'Permite al acudiente consultar pagos y estado de cuenta.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_DOCUMENTOS_VIEW,
    label: 'Acudiente ver documentos',
    description: 'Permite al acudiente descargar boletines, certificados u otros documentos.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_MESSAGES_SEND,
    label: 'Acudiente enviar mensajes',
    description: 'Permite al acudiente enviar mensajes a roles autorizados.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.ACUDIENTE_NOTIFICATIONS_VIEW,
    label: 'Acudiente ver notificaciones',
    description: 'Permite al acudiente visualizar notificaciones del portal.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.EMPLEADOS_VIEW,
    label: 'Ver empleados',
    description: 'Permite visualizar el modulo de empleados.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.EMPLEADOS_CREATE,
    label: 'Crear empleados',
    description: 'Permite crear empleados.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.EMPLEADOS_EDIT,
    label: 'Editar empleados',
    description: 'Permite editar empleados.',
  },
  {
    group: 'Miembros',
    key: PERMISSION_KEYS.EMPLEADOS_DELETE,
    label: 'Eliminar empleados',
    description: 'Permite eliminar empleados.',
  },
  {
    group: 'Plantel',
    key: PERMISSION_KEYS.PLANTEL_VIEW,
    label: 'Ver datos del plantel',
    description: 'Permite consultar los datos institucionales del plantel.',
  },
  {
    group: 'Plantel',
    key: PERMISSION_KEYS.PLANTEL_MANAGE,
    label: 'Gestionar datos del plantel',
    description: 'Permite editar y guardar cambios de los datos del plantel.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.ACADEMIC_SETUP_MANAGE,
    label: 'Gestion academica (general)',
    description: 'Permite gestionar configuraciones academicas (permiso general).',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVENTS_MANAGE,
    label: 'Gestionar eventos',
    description: 'Permite gestionar eventos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.CIRCULARS_MANAGE,
    label: 'Gestionar circulares',
    description: 'Permite gestionar circulares.',
  },
  {
    group: 'Participacion',
    key: PERMISSION_KEYS.VOTACIONES_MANAGE,
    label: 'Gestionar votaciones',
    description: 'Permite crear, editar, publicar y eliminar votaciones para la comunidad educativa.',
  },
  {
    group: 'Participacion',
    key: PERMISSION_KEYS.ENCUESTAS_MANAGE,
    label: 'Gestionar encuestas',
    description: 'Permite crear, editar, publicar y eliminar encuestas para la comunidad educativa.',
  },
  {
    group: 'Participacion',
    key: PERMISSION_KEYS.ACUDIENTE_VOTACIONES_VIEW,
    label: 'Acudiente ver votaciones',
    description: 'Permite al acudiente consultar y responder votaciones dirigidas a sus estudiantes vinculados.',
  },
  {
    group: 'Participacion',
    key: PERMISSION_KEYS.ACUDIENTE_ENCUESTAS_VIEW,
    label: 'Acudiente ver encuestas',
    description: 'Permite al acudiente consultar y responder encuestas dirigidas a sus estudiantes vinculados.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.SUBJECTS_MANAGE,
    label: 'Gestionar asignaturas',
    description: 'Permite gestionar asignaturas.',
  },
  {
    group: 'Reportes',
    key: PERMISSION_KEYS.REPORTS_VIEW,
    label: 'Ver reportes',
    description: 'Permite visualizar el modulo de reportes.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_VIEW,
    label: 'Ver pagos',
    description: 'Permite visualizar el modulo de pagos.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_IMPUESTOS_MANAGE,
    label: 'Gestionar impuestos',
    description: 'Permite crear y administrar impuestos.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_RESOLUCIONES_MANAGE,
    label: 'Gestionar resoluciones',
    description: 'Permite crear y administrar resoluciones.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_CAJA_MANAGE,
    label: 'Gestionar cajas',
    description: 'Permite crear y administrar cajas.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_DATOS_COBRO_MANAGE,
    label: 'Gestionar datos de cobro',
    description: 'Permite administrar la configuracion de datos de cobro.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_ITEM_COBRO_MANAGE,
    label: 'Gestionar items de cobro',
    description: 'Permite crear y administrar items de cobro.',
  },
  {
    group: 'Pagos',
    key: PERMISSION_KEYS.PAYMENTS_SERVICIOS_COMPLEMENTARIOS_MANAGE,
    label: 'Gestionar servicios complementarios',
    description: 'Permite crear y administrar servicios complementarios.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_CRM_VIEW,
    label: 'Ver CRM de admisiones',
    description: 'Permite visualizar el tablero y el listado de leads de admisiones.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_CRM_MANAGE,
    label: 'Gestionar CRM de admisiones',
    description: 'Permite crear leads, actualizar etapas y editar la ficha comercial.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_FOLLOWUPS_MANAGE,
    label: 'Gestionar seguimientos de admisiones',
    description: 'Permite registrar llamadas, notas y proximos seguimientos de cada lead.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_INTERVIEWS_MANAGE,
    label: 'Gestionar entrevistas de admisiones',
    description: 'Permite programar, registrar y actualizar entrevistas de leads.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_TASKS_MANAGE,
    label: 'Gestionar tareas de admisiones',
    description: 'Permite crear y cerrar tareas internas asociadas a un lead.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_REPORTS_VIEW,
    label: 'Ver reportes de admisiones',
    description: 'Permite consultar indicadores y reportes del CRM de admisiones.',
  },
  {
    group: 'Admisiones',
    key: PERMISSION_KEYS.ADMISSIONS_CONVERT_ENROLLMENT,
    label: 'Cerrar como matriculado',
    description: 'Permite cerrar un lead como matriculado dentro del CRM de admisiones.',
  },
  {
    group: 'WhatsApp',
    key: PERMISSION_KEYS.WHATSAPP_MODULE_VIEW,
    label: 'Ver modulo WhatsApp',
    description: 'Activa o oculta por completo el grupo y las pantallas de WhatsApp.',
  },
  {
    group: 'WhatsApp',
    key: PERMISSION_KEYS.WHATSAPP_SEND,
    label: 'Enviar mensajes WhatsApp',
    description: 'Permite enviar mensajes manuales de WhatsApp desde los modulos habilitados.',
  },
  {
    group: 'WhatsApp',
    key: PERMISSION_KEYS.WHATSAPP_TEMPLATES_MANAGE,
    label: 'Gestionar plantillas WhatsApp',
    description: 'Permite crear y editar plantillas de WhatsApp.',
  },
  {
    group: 'WhatsApp',
    key: PERMISSION_KEYS.WHATSAPP_CAMPAIGNS_MANAGE,
    label: 'Gestionar campanas WhatsApp',
    description: 'Permite crear y administrar campanas de WhatsApp.',
  },
  {
    group: 'WhatsApp',
    key: PERMISSION_KEYS.WHATSAPP_SETTINGS_MANAGE,
    label: 'Configurar WhatsApp',
    description: 'Permite administrar la configuracion del proveedor y credenciales de WhatsApp.',
  },
  {
    group: 'SMS',
    key: PERMISSION_KEYS.SMS_MODULE_VIEW,
    label: 'Ver modulo SMS',
    description: 'Activa o oculta por completo el grupo y las pantallas del modulo SMS.',
  },
  {
    group: 'SMS',
    key: PERMISSION_KEYS.SMS_SEND,
    label: 'Enviar SMS',
    description: 'Permite enviar SMS manuales o disparar envios rapidos desde otros modulos.',
  },
  {
    group: 'SMS',
    key: PERMISSION_KEYS.SMS_HISTORY_VIEW,
    label: 'Ver historial SMS',
    description: 'Permite consultar el historial y estado de los SMS enviados.',
  },
  {
    group: 'SMS',
    key: PERMISSION_KEYS.SMS_TEMPLATES_MANAGE,
    label: 'Gestionar plantillas SMS',
    description: 'Permite crear, editar y sembrar plantillas SMS.',
  },
  {
    group: 'SMS',
    key: PERMISSION_KEYS.SMS_SETTINGS_MANAGE,
    label: 'Configurar SMS',
    description: 'Permite administrar la configuracion del proveedor y credenciales SMS.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_MODULE_VIEW,
    label: 'Ver modulo de desempeno',
    description: 'Activa el grupo del menu y el acceso base al modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_DASHBOARD_VIEW,
    label: 'Ver dashboard de desempeno',
    description: 'Permite acceder al tablero principal del modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_PERIODS_VIEW,
    label: 'Ver periodos de desempeno',
    description: 'Permite consultar los periodos de evaluacion de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_PERIODS_CREATE,
    label: 'Crear periodos de desempeno',
    description: 'Permite crear nuevos periodos de evaluacion de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_PERIODS_EDIT,
    label: 'Editar periodos de desempeno',
    description: 'Permite editar periodos existentes del modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_PERIODS_CLOSE,
    label: 'Cerrar periodos de desempeno',
    description: 'Permite cambiar el estado de los periodos a cerrados o finalizados.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_TEMPLATES_VIEW,
    label: 'Ver plantillas de desempeno',
    description: 'Permite consultar las plantillas configuradas para evaluacion de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_TEMPLATES_CREATE,
    label: 'Crear plantillas de desempeno',
    description: 'Permite crear nuevas plantillas para roles y cargos.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_TEMPLATES_EDIT,
    label: 'Editar plantillas de desempeno',
    description: 'Permite editar plantillas de evaluacion de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_TEMPLATES_DELETE,
    label: 'Eliminar plantillas de desempeno',
    description: 'Permite eliminar plantillas del modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_ASSIGNMENTS_VIEW,
    label: 'Ver asignaciones de desempeno',
    description: 'Permite consultar las asignaciones del proceso de evaluacion de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_ASSIGNMENTS_CREATE,
    label: 'Crear asignaciones de desempeno',
    description: 'Permite crear nuevas asignaciones entre evaluador y evaluado.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_ASSIGNMENTS_EDIT,
    label: 'Editar asignaciones de desempeno',
    description: 'Permite actualizar asignaciones existentes del proceso de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_ASSIGNMENTS_DELETE,
    label: 'Eliminar asignaciones de desempeno',
    description: 'Permite eliminar asignaciones del proceso de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_EVALUATIONS_VIEW,
    label: 'Ver evaluaciones de desempeno',
    description: 'Permite consultar las evaluaciones de desempeno creadas a partir de asignaciones.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_EVALUATIONS_CREATE,
    label: 'Crear evaluaciones de desempeno',
    description: 'Permite iniciar diligenciamientos de evaluacion de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_EVALUATIONS_EDIT,
    label: 'Editar evaluaciones de desempeno',
    description: 'Permite modificar borradores o evaluaciones en proceso.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_EVALUATIONS_SUBMIT,
    label: 'Enviar evaluaciones de desempeno',
    description: 'Permite cerrar y enviar evaluaciones diligenciadas para consolidacion.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_RESULTS_VIEW,
    label: 'Ver resultados de desempeno',
    description: 'Permite consultar resultados consolidados del proceso de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_RESULTS_CREATE,
    label: 'Crear resultados de desempeno',
    description: 'Permite consolidar resultados a partir de evaluaciones enviadas.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_RESULTS_EDIT,
    label: 'Editar resultados de desempeno',
    description: 'Permite ajustar observaciones y datos de resultados consolidados.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_RESULTS_APPROVE,
    label: 'Aprobar resultados de desempeno',
    description: 'Permite aprobar oficialmente resultados consolidados del modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_IMPROVEMENT_VIEW,
    label: 'Ver planes de mejora',
    description: 'Permite consultar los planes de mejora asociados a resultados de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_IMPROVEMENT_CREATE,
    label: 'Crear planes de mejora',
    description: 'Permite crear planes de mejora a partir de resultados consolidados.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_IMPROVEMENT_EDIT,
    label: 'Editar planes de mejora',
    description: 'Permite actualizar acciones, seguimientos y compromisos de los planes de mejora.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_IMPROVEMENT_CLOSE,
    label: 'Cerrar planes de mejora',
    description: 'Permite cerrar o marcar como cumplidos los planes de mejora.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_HISTORY_VIEW,
    label: 'Ver historial de desempeno',
    description: 'Permite consultar la trazabilidad historica de evaluaciones, resultados y planes de mejora.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_REPORTS_VIEW,
    label: 'Ver reportes de desempeno',
    description: 'Permite acceder a los reportes e indicadores consolidados del modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_REPORTS_EXPORT,
    label: 'Exportar reportes de desempeno',
    description: 'Permite exportar a Excel los reportes del modulo de desempeno.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_OWN_VIEW,
    label: 'Ver mi desempeno',
    description: 'Permite al usuario institucional consultar sus autoevaluaciones, resultados y planes de mejora.',
  },
  {
    group: 'Desempeno',
    key: PERMISSION_KEYS.DESEMPENO_SELF_EVALUATE,
    label: 'Realizar autoevaluacion',
    description: 'Permite al usuario diligenciar sus autoevaluaciones dentro del modulo de desempeno.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.SCHEDULE_VIEW,
    label: 'Ver horario',
    description: 'Permite visualizar el modulo de horario.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.CERTIFICADOS_VIEW,
    label: 'Ver certificados',
    description: 'Permite visualizar el modulo de diplomas, boletines y certificados.',
  },
  {
    group: 'Certificados',
    key: PERMISSION_KEYS.CERTIFICADOS_GENERATE,
    label: 'Generar diplomas y certificados',
    description: 'Permite generar y descargar diplomas/certificados en PDF.',
  },
  {
    group: 'Certificados',
    key: PERMISSION_KEYS.CERTIFICADOS_ACADEMIC_YEAR_EDIT,
    label: 'Modificar año lectivo',
    description: 'Permite cambiar manualmente el año lectivo al generar diplomas y certificados.',
  },
  {
    group: 'Boletines',
    key: PERMISSION_KEYS.BOLETINES_VIEW,
    label: 'Ver boletines',
    description: 'Permite visualizar el modulo de boletines parciales y finales.',
  },
  {
    group: 'Boletines',
    key: PERMISSION_KEYS.BOLETINES_EDIT,
    label: 'Registrar notas de boletines',
    description: 'Permite ingresar y actualizar notas (desempeño/promedio) por periodo.',
  },
  {
    group: 'Boletines',
    key: PERMISSION_KEYS.BOLETINES_GENERATE,
    label: 'Generar boletines (PDF)',
    description: 'Permite generar y descargar boletines parciales o finales en PDF.',
  },
  {
    group: 'Configuracion',
    key: PERMISSION_KEYS.CONFIG_CERTIFICADOS_TEMPLATES_MANAGE,
    label: 'Gestionar plantillas de certificados',
    description: 'Permite configurar plantillas (fondo, textos y firmas) para diplomas y certificados.',
  },
  {
    group: 'Configuracion',
    key: PERMISSION_KEYS.CONFIG_BOLETINES_STRUCTURE_MANAGE,
    label: 'Configurar estructura de boletines',
    description: 'Permite configurar items a calificar por grado/grupo (grupos y subgrupos).',
  },
  {
    group: 'Notificaciones',
    key: PERMISSION_KEYS.NOTIFICATIONS_CREATE,
    label: 'Crear notificaciones',
    description: 'Permite crear y enviar notificaciones.',
  },
  {
    group: 'Asistencia',
    key: PERMISSION_KEYS.ASISTENCIA_VIEW,
    label: 'Ver asistencia',
    description: 'Permite ver el modulo de asistencia.',
  },
  {
    group: 'Asistencia',
    key: PERMISSION_KEYS.ASISTENCIA_CONFIG_MANAGE,
    label: 'Configurar asistencia',
    description: 'Permite administrar la configuracion de asistencia.',
  },
  {
    group: 'Asistencia',
    key: PERMISSION_KEYS.ASISTENCIA_DELETE,
    label: 'Borrar asistencia',
    description: 'Permite borrar (cambiar a No) una marcacion de asistencia desde el listado.',
  },
  {
    group: 'Mensajes',
    key: PERMISSION_KEYS.MESSAGES_DELETE,
    label: 'Eliminar mensajes',
    description: 'Permite eliminar mensajes en el modulo de mensajeria.',
  },
  {
    group: 'Mensajes',
    key: PERMISSION_KEYS.MESSAGES_SEND,
    label: 'Enviar mensajes',
    description: 'Permite redactar y enviar mensajes a otros usuarios.',
  },
  {
    group: 'Mensajes',
    key: PERMISSION_KEYS.MESSAGES_REPLY,
    label: 'Responder mensajes',
    description: 'Permite responder mensajes recibidos.',
  },
  {
    group: 'Mensajes',
    key: PERMISSION_KEYS.MESSAGES_READ_RECEIPTS_VIEW,
    label: 'Ver leidos',
    description: 'Permite ver el estado (leido/no leido) de los destinatarios en mensajes enviados.',
  },
  {
    group: 'Mensajes',
    key: PERMISSION_KEYS.CHAT_ONLINE_VIEW,
    label: 'Chat en linea',
    description: 'Permite acceder al chat en linea dentro de la plataforma.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.TASKS_VIEW,
    label: 'Ver tareas',
    description: 'Permite visualizar el modulo de tareas.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.TASKS_CREATE,
    label: 'Crear tareas',
    description: 'Permite crear nuevas tareas.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.TASKS_EDIT,
    label: 'Editar tareas',
    description: 'Permite modificar tareas existentes.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.TASKS_DELETE,
    label: 'Eliminar tareas',
    description: 'Permite eliminar tareas.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.TASKS_REPLY,
    label: 'Responder a entregas',
    description: 'Permite enviar retroalimentacion y calificar entregas de tareas.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_VIEW,
    label: 'Ver evaluaciones',
    description: 'Permite visualizar el modulo de evaluaciones.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_MANAGE,
    label: 'Gestionar evaluaciones (general)',
    description: 'Permiso general que otorga acceso completo a todas las acciones de evaluaciones.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_CREATE,
    label: 'Crear evaluaciones',
    description: 'Permite crear nuevas evaluaciones.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_EDIT,
    label: 'Editar evaluaciones',
    description: 'Permite modificar evaluaciones existentes.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_DELETE,
    label: 'Eliminar evaluaciones',
    description: 'Permite eliminar evaluaciones.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_FOLLOW_UP,
    label: 'Ver seguimiento de evaluaciones',
    description: 'Permite consultar el estado y seguimiento de evaluaciones.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_TAKE,
    label: 'Realizar evaluacion',
    description: 'Permite a los estudiantes realizar evaluaciones en linea.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.EVALUATIONS_GRADE,
    label: 'Calificar evaluacion',
    description: 'Permite calificar y registrar notas de evaluaciones.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.SCHEDULE_EDIT,
    label: 'Editar horario',
    description: 'Permite realizar cambios en la programacion de horario.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.PERMISSIONS_MANAGE,
    label: 'Gestionar permisos',
    description: 'Permite configurar los permisos por rol.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_CHAT_MANAGE,
    label: 'Configuracion de chat',
    description: 'Permite configurar reglas de chat por rol/grupo.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_MAIL_SERVER_MANAGE,
    label: 'Datos del servidor de correo',
    description: 'Permite configurar los datos del servidor SMTP para el envio interno de correos.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_MESSAGES_MANAGE,
    label: 'Configuracion de mensajes',
    description: 'Permite configurar reglas de mensajeria por rol/grupo.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_NOTIFICATIONS_MANAGE,
    label: 'Configuracion de notificaciones',
    description: 'Permite configurar reglas de notificaciones por rol/grupo.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_REPORT_TYPES_MANAGE,
    label: 'Configuracion tipos de reporte',
    description: 'Permite configurar a que roles aplica cada tipo de reporte.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_TIPO_PERMISOS_MANAGE,
    label: 'Tipos de permiso',
    description: 'Permite administrar los tipos de permiso.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_TIPO_INASISTENCIAS_MANAGE,
    label: 'Tipos de inasistencia',
    description: 'Permite administrar los tipos de inasistencia.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_TIPO_CERTIFICADO_MANAGE,
    label: 'Tipo de certificado',
    description: 'Permite administrar los tipos de certificado.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_TIPO_EMPLEADO_MANAGE,
    label: 'Tipo empleado',
    description: 'Permite crear y administrar tipos de empleado.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.CONFIG_LINKED_DEVICES_VIEW,
    label: 'Dispositivos vinculados',
    description: 'Permite ver y administrar las sesiones y dispositivos vinculados de la cuenta.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.ANNOUNCEMENTS_MANAGE,
    label: 'Gestionar anuncios',
    description: 'Permite crear, editar y eliminar anuncios para mostrarlos en el inicio.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.ROLES_MANAGE,
    label: 'Administrar roles',
    description: 'Permite crear, editar, eliminar y ver roles, asegurando que los roles predeterminados esten protegidos.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.STORAGE_MANAGE,
    label: 'Administrar almacenamiento',
    description: 'Permite visualizar y gestionar el almacenamiento y los archivos del plantel.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.BULK_UPLOAD_MANAGE,
    label: 'Cargue masivo',
    description: 'Permite realizar importacion masiva de datos mediante archivos.',
  },
  {
    group: 'Configuración',
    key: PERMISSION_KEYS.EXPORT_EXCEL,
    label: 'Exportar a Excel',
    description: 'Permite exportar datos a formato Excel en todos los modulos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.INASISTENCIAS_VIEW,
    label: 'Ver inasistencias',
    description: 'Permite visualizar reportes y tipos de inasistencias.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.INASISTENCIAS_CREATE,
    label: 'Crear inasistencias',
    description: 'Permite reportar nuevas inasistencias y tipos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.INASISTENCIAS_EDIT,
    label: 'Editar inasistencias',
    description: 'Permite modificar inasistencias reportadas y tipos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.INASISTENCIAS_DELETE,
    label: 'Eliminar inasistencias',
    description: 'Permite eliminar inasistencias y tipos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.PERMISOS_VIEW,
    label: 'Ver permisos',
    description: 'Permite visualizar solicitudes y tipos de permisos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.PERMISOS_CREATE,
    label: 'Crear permisos',
    description: 'Permite solicitar nuevos permisos y crear tipos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.PERMISOS_EDIT,
    label: 'Editar permisos',
    description: 'Permite modificar solicitudes de permisos y tipos.',
  },
  {
    group: 'Academico',
    key: PERMISSION_KEYS.PERMISOS_DELETE,
    label: 'Eliminar permisos',
    description: 'Permite eliminar permisos solicitados y tipos.',
  },
]

const DEFAULT_ROLE_PERMISSIONS = {
  administrador: [],
  directivo: [],
  profesor: [],
  estudiante: [],
  aspirante: [],
  acudiente: [],
}

function normalizeRolePermissionsData(rawData) {
  const normalized = {}
  Object.keys(DEFAULT_ROLE_PERMISSIONS).forEach((roleName) => {
    const incoming = rawData?.[roleName]
    normalized[roleName] = Array.isArray(incoming)
      ? incoming
        .filter((permission) => typeof permission === 'string')
        .map((permission) => permission.trim())
        .filter(Boolean)
      : []
  })

  Object.entries(rawData || {}).forEach(([roleName, permissions]) => {
    if (normalized[roleName] !== undefined) return
    normalized[roleName] = Array.isArray(permissions)
      ? permissions
        .filter((permission) => typeof permission === 'string')
        .map((permission) => permission.trim())
        .filter(Boolean)
      : []
  })
  return normalized
}

function resolveRolePermissions(role, rolePermissionsMap) {
  const normalizedRole = String(role || '').toLowerCase()
  const source = rolePermissionsMap?.[normalizedRole]
  if (!Array.isArray(source)) return []
  return source
}

function hasRolePermission(role, permissionKey, rolePermissionsMap) {
  return resolveRolePermissions(role, rolePermissionsMap).includes(permissionKey)
}

function buildDynamicMemberPermissionKey(roleId, action) {
  const normalizedId = String(roleId || '').trim()
  const normalizedAction = String(action || '').trim().toLowerCase()
  if (!normalizedId) return 'members_dynamic_role__invalid__'

  const allowed = new Set(['view', 'create', 'edit', 'delete'])
  if (!allowed.has(normalizedAction)) return `members_dynamic_role_${normalizedId}__invalid__`

  return `members_dynamic_role_${normalizedId}_${normalizedAction}`
}

export {
  DEFAULT_ROLE_PERMISSIONS,
  PERMISSION_KEYS,
  PERMISSIONS_CATALOG,
  PROTECTED_ROLE_VALUES,
  ROLE_OPTIONS,
  buildAllRoleOptions,
  buildDynamicMemberPermissionKey,
  hasRolePermission,
  normalizeRolePermissionsData,
  resolveRolePermissions,
}
