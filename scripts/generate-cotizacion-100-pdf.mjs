...
addHeader("Cotizacion Comercial - 100 Estudiantes", "Fecha: 03 de junio de 2026 | Vigencia: 30 dias");
let y = 48;
section("Resumen ejecutivo", y);
y += 7;
body("Propuesta referencial para una institucion educativa de aproximadamente 100 estudiantes, bajo el plan Esencial de Edupleace y modelo SaaS por estudiante activo.", y);
y += 14;

const metricGap = 4;
const metricW = (contentWidth - metricGap * 3) / 4;
labelValue("Plan recomendado", "Esencial", left, y, metricW);
labelValue("Estudiantes", "100", left + metricW + metricGap, y, metricW);
labelValue("Mensualidad", "COP 650.000", left + (metricW + metricGap) * 2, y, metricW);
labelValue("Implementacion", "COP 1M a 2M", left + (metricW + metricGap) * 3, y, metricW);
y += 31;
...
doc.setFont("helvetica", "bold");
doc.setFontSize(9);
doc.text("Calculo mensual: COP 350.000 de cargo fijo + 100 estudiantes x COP 3.000 = COP 650.000 / mes.", left + 3, y + 9, { maxWidth: contentWidth - 6 });
y += 27;
...
autoTable(doc, {
  ...
  body: [
    ["Cargo fijo mensual", "Acceso base al plan Esencial, soporte funcional basico, mantenimiento correctivo y actualizaciones menores.", "COP 350.000 / mes"],
    ["Valor por estudiante", "100 estudiantes activos estimados. Acudientes incluidos sin cargo adicional.", "COP 3.000 por estudiante / mes"],
    ["Total mensual estimado", "Modelo: cargo fijo mensual + estudiantes activos matriculados.", "COP 650.000 / mes"],
    ["Implementacion inicial", "Parametrizacion, configuracion inicial, acompanamiento funcional y puesta en marcha segun alcance acordado.", "COP 1.000.000 a 2.000.000"],
  ],
});

y = doc.lastAutoTable.finalY + 10;
section("Modulos incluidos en el plan Esencial", y);
autoTable(doc, {
  startY: y + 5,
  margin: { left, right, bottom: 18 },
  theme: "grid",
  ...
  body: [
    ["Gestion institucional", "Estudiantes, profesores, directivos, usuarios y datos del plantel."],
    ["Comunicacion", "Mensajes y notificaciones institucionales."],
    ["Seguridad y acceso", "Roles basicos y usuarios internos cubiertos hasta el limite del plan."],
...
y += 7;
body("El plan Esencial cubre hasta 25 profesores y 8 directivos o administrativos. Los acudientes estan incluidos sin costo adicional dentro del modelo por estudiante.", y);
y += 16;
...
y += 7;
body("Incluye acceso a la plataforma segun el plan Esencial, soporte funcional basico, mantenimiento correctivo, actualizaciones menores y parametrizacion inicial segun el alcance acordado.", y);
y += 13;
...
autoTable(doc, {
  ...
  body: [
    ...
    ["Academico", "COP 500.000 / mes", "Desde COP 3.800", "200 a 600 estudiantes", "Hasta 45 profesores y 12 admin.", "COP 2M a 6M"],
    ["Institucional", "COP 750.000 / mes", "Desde COP 4.500", "300 a 900 estudiantes", "Hasta 70 profesores y 20 admin.", "COP 6M a 10M"],
    ["Integral", "COP 1.100.000 / mes", "Desde COP 5.500", "400 a 1.500 o mas", "Hasta 100 profesores y 30 admin.", "COP 10M a 25M"],
  ],
});
...
const outputPath = "D:/plataformaescolar/COTIZACION_COMERCIAL_100_ESTUDIANTES.pdf";
const fallbackPath = "D:/plataformaescolar/COTIZACION_COMERCIAL_100_ESTUDIANTES_ACTUALIZADA.pdf";
const timestampPath = `D:/plataformaescolar/COTIZACION_COMERCIAL_100_ESTUDIANTES_ACTUALIZADA_${new Date().toISOString().replace(/[-:T.Z]/g, "").slice(0, 14)}.pdf`;
