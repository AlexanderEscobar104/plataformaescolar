...
addHeader("Cotizacion Comercial - 150 Estudiantes", "Fecha: 05 de junio de 2026 | Vigencia: 30 dias");
let y = 48;
section("Resumen ejecutivo", y);
y += 7;
body("Propuesta referencial para una institucion educativa de aproximadamente 150 estudiantes, bajo el plan Esencial de Edupleace y modelo SaaS por estudiante activo.", y);
y += 14;

const metricGap = 4;
const metricW = (contentWidth - metricGap * 3) / 4;
labelValue("Plan recomendado", "Esencial", left, y, metricW);
labelValue("Estudiantes", "150", left + metricW + metricGap, y, metricW);
labelValue("Mensualidad", "COP 800.000", left + (metricW + metricGap) * 2, y, metricW);
...
doc.setFont("helvetica", "bold");
doc.setFontSize(9);
doc.text("Calculo mensual: COP 350.000 de cargo fijo + 150 estudiantes x COP 3.000 = COP 800.000 / mes.", left + 3, y + 9, { maxWidth: contentWidth - 6 });
y += 27;
...
autoTable(doc, {
  ...
  body: [
    ...
    ["Valor por estudiante", "150 estudiantes activos estimados. Acudientes incluidos sin cargo adicional.", "COP 3.000 por estudiante / mes"],
    ["Total mensual estimado", "Modelo: cargo fijo mensual + estudiantes activos matriculados.", "COP 800.000 / mes"],
...
const outputPath = "D:/plataformaescolar/COTIZACION_COMERCIAL_150_ESTUDIANTES.pdf";
const fallbackPath = "D:/plataformaescolar/COTIZACION_COMERCIAL_150_ESTUDIANTES_ACTUALIZADA.pdf";
const timestampPath = `D:/plataformaescolar/COTIZACION_COMERCIAL_150_ESTUDIANTES_ACTUALIZADA_${new Date().toISOString().replace(/[-:T.Z]/g, "").slice(0, 14)}.pdf`;
